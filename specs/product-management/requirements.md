...
- **RF03.2 (Query Product & Stock)**: System must allow retrieving product details and current stock balance (`GET /api/v1/produtos/{id}/estoque`).
...
- **RF03.4 (Stock Adjustment)**: System must allow registering stock adjustments (`POST /api/v1/produtos/{id}/estoque/ajustes`) with type (`ENTRY` or `EXIT`), quantity (> 0), and reason.
- **RF03.5 (Deactivate Product)**: System must allow logical deactivation (`DELETE /api/v1/produtos/{id}`).
- **RF10 (Movement Traceability)**: System must allow querying product movement records (`GET /api/v1/produtos/{id}/movimentacoes`).
